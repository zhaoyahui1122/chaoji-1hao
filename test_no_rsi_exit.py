...
configs = [
    {"name": "自适应(无RSI出场)", "adx": 25.0},
    {"name": "纯均值回归(无RSI出场)", "adx": 50.0},
]
...
for cfg in configs:
    resp = requests.post("http://127.0.0.1:8012/backtest", json={
        ...
        "turtle_atr_filter": 0.0, "turtle_adx_period": 14, "turtle_adx_threshold": cfg["adx"],
    ...
    print(f"{'=' * 60}")
    print(f"{cfg['name']} | 5分钟 | SL=2.5% TP=3.75% | Maker费率")
    print(f"{'=' * 60}")
