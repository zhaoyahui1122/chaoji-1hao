...
r = requests.post("http://127.0.0.1:8012/backtest", json={
    "strategy_type": "turtle", "symbol": "BTC_USDT", "timeframe": "15m", "data_source": "gate",
...
eq = r.json().get("equity_curve", [])

print("=" * 60)
print("自适应策略 15分钟 | 盈亏比 1:1.5 | Maker费率")
print("=" * 60)
print(f"K线数: {len(eq)}")
...
print(f"实际盈亏比: 1:{abs(avg_win/avg_loss):.2f}" if avg_loss != 0 else "")
print()
